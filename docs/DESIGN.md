...
**How we deliver our services** keeps Workshops, Workflows and Automations fully readable. Each article explains the customer's starting material, Found42's work, participation or review, and the result. Its **Discuss…** action names the published service in the dialog. The live contact form uses its own Training/Automation interest vocabulary, so the handoff also gives visitors the service name to carry into that form. An inquiry requests a conversation; it is not a booked meeting. The homepage keeps this section; the Services page no longer repeats it.

## Services page: the catalog

`/services/` presents Found42's service catalog (`serviceCatalog` in `src/content.ts`) instead of repeating the homepage. It opens with jump links, then: **five training tracks** (C-Level AI, Analysts, GTM Teams, AI Builders, Customized Role-Based Training), each with its audience, what participants leave with, its format and a **Discuss…** action that names the track in the live-form handoff; **formats and pricing**; **beyond training** (custom build, advisory retainer, advisory facilitation, annual program); **two free 30-minute sessions** on the warm ground; and attributed client quotes.

The price list is the page's one emphatic element: starting prices set large in tabular display figures, right-aligned. Only a starting price is published ("From $599 per seat"); options without one read "Quoted after discovery", and every engagement gets a fixed quote after a free discovery assessment. The catalog's upper bounds and anything from its client-specific edition (client name, committee, NDA terms, confidentiality and validity notices, client-specific recommendation) are deliberately not published. On narrow screens each price row stacks; explicit table roles keep the rows and headers announced.
