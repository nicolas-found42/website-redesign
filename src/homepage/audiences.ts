...
export function audiencesSection() {
  const rail = audiences
    .map(
      (audience, index) =>
        `<button class="audience-choice" type="button" data-audience="${index}" aria-pressed="${index === 0}"><span class="audience-choice-name">${audience.choice}</span><span class="audience-choice-note note--plain">${audience.proposition}</span></button>`,
    )
    .join("");

  const panels = audiences
    .map(
      (audience, index) => `<article class="audience-panel" id="audience-${audience.id}" data-audience-panel="${index}" aria-labelledby="audience-${audience.id}-title">
   <div class="audience-copy">
    ...
    ${audience.linkNote ? `<p class="note--plain audience-link-note">${audience.linkNote}</p>` : ""}
    ${audience.link.href
      ? `<a class="link" href="${audience.link.href}">${audience.link.label} <span class="signal-dot"></span>${icon("arrow")}</a>`
      : `<button class="link" data-dialog="contact" data-service="AI builder support" data-interest="Automation" data-contact="ai-builder-support">${audience.link.label} <span class="signal-dot"></span>${icon("arrow")}</button>`}
   </div>
