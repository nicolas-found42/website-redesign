...
export function servicesSection() {
 ...
 <div class="service-list">${services.map((service, index) => `<article class="service-row reveal"><div class="service-art" aria-hidden="true">${ribbonSvg(index, { className: "ribbon-motif" })}<span>0${index + 1}</span></div><div><p class="service-label">${service.label}</p><h3>${service.title}</h3></div><p>${service.description}</p></article>`).join("")}</div>
