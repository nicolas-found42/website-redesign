...
/**
 * The playbook entry shows what it is about rather than only saying it: the
 * review scene the resources page draws, told when it comes into view.
 */
...
export function resourcesSection() {
  ...
  const entry = (r: typeof first, i: number) => {
    ...
    return r.id === "playbook"
      ? `<article class="resource resource--figure" data-reveal><div class="resource-copy">${copy}</div>${reviewFigure}</article>`
      : `<article class="resource" data-reveal>${copy}</article>`;
  };
  return `<section id="resources" class="band wrap resources" aria-labelledby="resources-title"><div class="section-head"><div><p class="index"><b>01</b><span class="rule"></span><span class="note">Free resources</span></p><h2 id="resources-title" class="display" data-reveal-lines>Useful before the call.</h2></div><p class="lead" data-reveal>Start by assessing your AI readiness. Then explore the course, reusable skills and failure-mode guidance. Access and preview limits are shown at each step.</p></div><div class="resource-grid"><article class="feature on-ink" data-reveal>${accentSvg("reading", "feature-accent")}<div class="feature-cover"><p class="note feature-kicker">Free working set · Index / 04</p><p class="feature-type">Start with<br><em>real work.</em></p><p class="note feature-foot">Four ways to start with real work.</p></div><div class="feature-copy"><h3>${first.title}</h3><p class="body">${first.description}</p><div class="access"><p class="note">${first.outcome}</p><p class="note--plain">${first.gate}</p></div>${link(first)}</div></article><div class="resource-column">${rest.map(entry).join("")}</div></div><div class="resource-foot"><div><h3>Start with value. Build what proves useful.</h3><p class="body">Each resource reveals where a tailored workflow can earn its keep.</p></div></div></section>`;
}
