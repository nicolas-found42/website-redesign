...
function industryPage(key: keyof typeof industries) {
  ...
  return (
    opening(
      ...
      d.title,
      d.intro,
      `<p class="page-number">${d.aside}</p><h2 class="heading-h3">${d.asideTitle}</h2>${d.asideBody ? `<p>${d.asideBody}</p>` : ""}`,
    ) +
    `<section class="band on-ink" data-ground="ink"><div class="wrap"><div class="section-head"><div>${index("Where we work")}<h2 class="display">${d.heading}</h2></div><p class="lead">${d.context}</p></div><div class="industry-grid">${d.items.map(([t, b], i) => `<article><p class="note">0${i + 1}</p><h3>${t}</h3><p>${b}</p></article>`).join("")}</div></div></section>${founder ? founderBand(founder) : ""}<section class="wrap band content-split"><div>${index("Built around your company")}<h2 class="display">Context in.<br>Judgment throughout.</h2><p>Built around your role, your industry, and your company—not a generic AI curriculum.</p>${"target" in d ? `<div class="industry-target"><p class="page-number">${d.target.figure}</p><h3 class="heading-h3">${d.target.title}</h3><p>${d.target.body}</p><p class="note--plain">A target, not a guaranteed result.</p></div>` : ""}<a class="link" href="${sitePath("services/")}">Workshops, Workflows & Automations&nbsp;→</a><a class="link" href="${sitePath("resources/#scorecard")}">Take the AI Readiness Scorecard&nbsp;→</a></div><div class="page-drawing">${schematicFigure(masterSchematic, "portrait")}</div></section>` +
