...
function resourcesPage() {
  return (
    ...
    `<section id="scorecard" class="wrap band"><div class="content-split"><div>${index("01 / AI Readiness Scorecard")}<h2 class="display">Know where you stand.</h2><p>The Found42 assessment asks about your current AI tools, data practices, workflow efficiency, integration readiness and automation goals. ScoreApp describes a five-minute assessment with a personalized report and recommendations.</p><p class="note--plain">ScoreApp requires your first and last name, email, company and country before the questions. Its privacy and communications terms apply. Report delivery has not been tested in this preview.</p></div><div class="resource-access"><h3>Take the original assessment</h3><p>Load the assessment here, or open it directly. Your answers and results stay with the assessment provider.</p><button class="action" data-load-scorecard disabled data-await-script>Load the scorecard here →</button><a class="link" href="https://found42.scoreapp.com/">Open the scorecard on ScoreApp →</a><p class="note--plain" data-scorecard-status role="status">The external assessment loads only when you choose to open it here.</p></div></div><div data-scorecard-frame></div><details class="workflow-preview"><summary>Try the four-question workflow preview · No email required</summary><p>From the redesign prototype: test one workflow against repetition, output clarity, review safety and frequency. This local discussion aid is separate from the AI Readiness Scorecard and does not generate its report.</p><div id="assessment" class="assessment" aria-label="Workflow discussion preview"></div></details></section>` +
    resources
      .slice(1)
      .map(
        (r, i) =>
