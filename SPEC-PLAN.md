...
This local specification is the planning artifact. GitHub Issues and the default triage labels are configured; see `docs/agents/issue-tracker.md` and `docs/agents/triage-labels.md`. Domain vocabulary is organized through `CONTEXT-MAP.md`. The exact issue title, body, `ready-for-agent` label, and relationships must be presented for confirmation before an external write. No issue has been published.
